...
export function inicializarSelectProductos() {
	...
	if (productoSelect instanceof HTMLSelectElement) {
		...
		productos.forEach((producto) => {
			...
			option.textContent =
				producto.nombre[0].toUpperCase() + producto.nombre.slice(1);
...
export function calcularTicket() {
	...
	if (
		productoSelect instanceof HTMLSelectElement &&
		...
		if (!nombreProducto || isNaN(cantidad) || cantidad <= 0) {
			...
			if (!productoEncontrado) {
				...
				nuevaFila.innerHTML = `
                    ...
                    <td>${(
											obtenerPorcentajeIva(productoEncontrado.tipoIva) * 100
										).toFixed(0)}%</td>
                    <td>${(
											productoEncontrado.precio *
											obtenerPorcentajeIva(productoEncontrado.tipoIva)
										).toFixed(2)} €</td>
                    <td>${iva.toFixed(2)} €</td>
...
export function actualizarTotales() {
	...
	if (
		...
		console.error(
			"Alguno de los elementos para mostrar totales no existe o no es un nodo HTML válido."
		);
	}
}
